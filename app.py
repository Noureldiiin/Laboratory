...
def add_patient():
    if request.method == 'POST':
        ...
        patient_diagnoses = request.form['diagnoses']
        patient_details = request.form['patient_details']

        add_patient(patient_name, patient_age, patient_gender, patient_email, patient_phone, patient_diagnoses, patient_details)
...
# Helper functions
def add_patient(patient_name, patient_age, patient_gender, patient_email, patient_phone, patient_diagnoses, patient_details):
...
def search_patients(search_term):
    ...
    for key in keys:
        patient = redis_client.hgetall(key)
        if (
            search_term.lower() in patient['name'].lower() or
            search_term.lower() in patient['details'].lower() or
            search_term.lower() in patient['diagnoses'].lower()
        ):
